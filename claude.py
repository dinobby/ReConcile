# credit to: https://github.com/KoushikNavuluri/Claude-API/blob/main/claude-api/claude_api.py
...
class Client:
  ...
  def send_message(self, prompt, conversation_id, attachment=None,timeout=500):
    ...
    account = self.cookie.split("--")[-1]
    if response.status_code == 429:    
        print(f"account {account} is out of usage. (status code: 429)")
        return None
    decoded_data = response.content.decode("utf-8")
    # if "error" in decoded_data:
    #     print(f"account {account} is out of usage.")
    #     print(f"error msg: {decoded_data}")
    #     return None
    decoded_data = re.sub('\n+', '\n', decoded_data).strip()
